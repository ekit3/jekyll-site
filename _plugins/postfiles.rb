...
module Jekyll
  ...
  # override trigger_hooks for excerpts, to actually do it !
  class Excerpt
  ...
  module PostFiles
    ...
    Jekyll::Hooks.register :posts, :pre_render do |post, hash|
      post.to_s.gsub!(/!\[(.*?)\]\((.*?)\)/) do |match|
        "![#{$1}](#{hash.site["baseurl"]}#{hash.page.url}/#{$2})"
      end
    end

    Jekyll::Hooks.register :excerpts, :pre_render do |excerpt, hash|
      excerpt.content.gsub!(/!\[(.*?)\]\((.*?)\)/) do |match|
        "![#{$1}](#{hash.site["baseurl"]}#{hash.page.url}/#{$2})"
      end
    end
